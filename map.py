...
if map_type == "My Location":
    st.header("Malaysia Map with Coordinate Highlight")
    st.write("A map focused on Malaysia with a marker at the coordinates of Universiti Teknologi Petronas.")

    # Coordinates of UTP
    utp_coords = (4.387391, 100.965044)

    # Create a folium map centered on Malaysia
    m = folium.Map(location=utp_coords, zoom_start=8)

    # Add a marker for UTP
    folium.Marker(
        location=utp_coords,
        popup="Universiti Teknologi Petronas",
        tooltip="UTP (4.387391, 100.965044)",
        icon=folium.Icon(color="red", icon="home")
    ).add_to(m)

    # Add a GeoJSON layer to show Malaysia borders if needed
    try:
        # Filter to only show Malaysia's borders
        malaysia_gdf = gdf[gdf['SOVEREIGNT'] == 'Malaysia']
        folium.GeoJson(malaysia_gdf, name="Malaysia Border").add_to(m)
    except Exception as e:
        st.write("Could not load Malaysia's borders:", e)
